...
def test_logs_perf_controls_and_measurement_profile_runtime_ui_exist():
    ...
    assert 'let LAST_ANALYSIS_CACHE_REUSE = null;' in index_body
    assert 'function _analysisMaybeReuseWindow(base, types, cacheStrategy){' in index_body
    assert 'id="analysis_log_filter_wrap"' in index_body or 'analysis_log_filter_wrap' in index_body
    ...
    assert "const buttons = Array.from(root.querySelectorAll('#influxbro_popup_radial_toggle'));" in tooltips_body
    assert "buttons.forEach((it)=>{ try{ if(it !== keep) it.remove(); }catch(e){} });" in tooltips_body
...
def test_iphone_topbar_safe_area_is_covered_without_double_offset():
    topbar = (Path(__file__).resolve().parents[1] / "influxbro" / "app" / "templates" / "_topbar.html").read_text()
...
def test_standard_tooltip_has_shift_hold_pin_and_doc_button():
    ...
    assert "document.querySelectorAll('dialog,[data-dialog-root=\"1\"],[role=\"dialog\"],.dlg_backdrop,.modal')" in tooltips
...
def test_dialog_close_reenables_tooltips():
    ...
    assert "function _notifyDialogOpened(root)" in dialog
    assert "influxbro:dialog-closed" in dialog
    assert "influxbro:dialog-opened" in dialog
    assert "_notifyDialogOpened(root);" in dialog
...
def test_raw_table_uses_outlier_type_labels_and_changes_button():
    index_html = (Path(__file__).resolve().parents[1] / "influxbro" / "app" / "templates" / "index.html").read_text()
    tooltips = (Path(__file__).resolve().parents[1] / "influxbro" / "app" / "templates" / "_tooltips.html").read_text()
