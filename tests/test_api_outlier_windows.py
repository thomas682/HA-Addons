...
def test_outlier_windows_resolves_actual_index_from_time_even_if_point_index_is_wrong(load_app_module, tmp_path, monkeypatch):
    app_mod = load_app_module(config_dir=tmp_path / "config", data_dir=tmp_path / "data")
    client = app_mod.app.test_client()
    ...
    base = datetime(2026, 4, 13, 8, 0, tzinfo=timezone.utc)
    points = [base + timedelta(minutes=idx) for idx in range(6)]
    ...
    assert j["ok"] is True
    assert len(j["windows"]) == 1
    assert j["windows"][0]["point_index"] == 4
    assert j["windows"][0]["before_time"] == "2026-04-13T08:02:00.000Z"
    assert j["windows"][0]["after_time"] == "2026-04-13T08:05:00.000Z"
