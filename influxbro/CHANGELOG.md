...
- Mobile / iOS: Vollständige iPhone-Optimierung für alle Seiten. `viewport-fit=cover` in allen 21 Templates ergänzt, damit Safe-Area-Insets auf Geräten mit Notch und Dynamic Island wirksam werden. `100vh` durch `100dvh` ersetzt, sodass das Layout beim Öffnen der virtuellen Tastatur stabil bleibt. Statusleiste (`errors_main.panel_statusbar`) reicht jetzt bis zum Bildschirmrand und schiebt ihren Inhalt via `padding-bottom` über den Home-Indicator. Dialoge und Modals berücksichtigen Safe-Area-Insets auf allen Seiten. iOS Auto-Zoom beim Tippen in Eingabefelder verhindert (min. 16px font-size ab 980px Breakpoint). Touch-Targets auf mindestens 44px angehoben (Apple HIG). `touch-action: manipulation` auf allen interaktiven Elementen gesetzt.
