...
- Standard-Tooltips erscheinen nur noch auf wirklich interaktiven Controls wie Buttons, Checkboxen, Eingabefeldern, Auswahlfeldern und Textfeldern. Links zeigen Tooltips nur, wenn sie explizit mit `data-tooltip-allow="1"` freigegeben sind. Der Topbar-Button `Tooltips` schaltet sie seitenuebergreifend ein oder aus. Bei einem sichtbaren Tooltip friert `Shift` den Tooltip ein; ein blaues Label zeigt den Freeze-Zustand, erneutes `Shift`, `Esc` oder ein Aussenklick loest ihn wieder. Picker, S-Picker und sichtbare Dialoge unterdruecken Tooltips weiterhin.
...
- Standarddialoge koennen am Kopfbereich verschoben und unten rechts ueber den sichtbaren Handle vergroessert oder verkleinert werden. Ein Doppelklick auf den Resize-Handle setzt Groesse und Position zurueck.
...
- Wenn die Doku aus einem Dialog geoeffnet wird, erscheint der Handbuchdialog ueber dem aktuellen Dialog und zeigt nur den Handbuch-Hauptinhalt ohne Topbar, Sidebar oder sonstige App-Navigation.
