...
Die vier Auswahlfelder `_measurement`, `_field`, `friendly_name` und `entity_id` verwenden dieselbe Kaskadenlogik wie auf der Backup-Seite. Wenn du eines der Felder aenderst, werden die anderen Listen sofort mit den gefilterten Datenbankwerten neu geladen. Der Zeitraum beeinflusst diese vier Vorschlagslisten nicht; er steuert nur die spaetere Datenabfrage fuer Graph, Tabelle und Statistik. Fuer Selector-Requests ohne explizites `range/start/stop` wird serverseitig All-Time verwendet (Start `1970-01-01T00:00:00Z`), damit Vorschlagslisten und `resolve_signal` konsistent den gesamten Datenbestand sehen. Wenn du die Vorschlaege bewusst begrenzen willst (Performance bei grossen Buckets), gib `range` explizit an (z.B. `24h`, `7d`). Beim Dashboard-Start werden die Selector-Listen einmal deterministisch geladen; ein zusaetzlicher zeitversetzter Hintergrund-Reload wird nicht mehr gestartet.
