...
- Standard-Tooltips erscheinen nur noch auf wirklich interaktiven Controls wie Buttons, Checkboxen, Eingabefeldern, Auswahlfeldern und Textfeldern. Links zeigen Tooltips nur, wenn sie explizit mit `data-tooltip-allow="1"` freigegeben sind. Der Topbar-Button `Tooltips` schaltet sie seitenuebergreifend ein oder aus. Bei einem sichtbaren Tooltip weist die kleine Fusszeile auf `Shift` hin; `Shift` friert den Tooltip ein, ein blaues Label zeigt den Freeze-Zustand, erneutes `Shift`, `Esc` oder ein Aussenklick loest ihn wieder. Picker, S-Picker und sichtbare Dialoge unterdruecken Tooltips weiterhin.
