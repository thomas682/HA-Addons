...
- Zusaetzlich besitzt der Desktop-Layout-Scrollbereich eine Mindesthoehe, damit der Inhalt nicht komplett verschwinden kann, falls die Topbar/Pagecard-Hoehenvariablen kurzzeitig falsch gemessen werden.
...
- Enthalten:
  - Profil-Auswahl inkl. `Anwenden`, `Speichern`, aktuelle Version und Anzeige des aktiven Farbschemas (`Thema: ...`). Das Farbschema kann direkt daneben in der Topbar weitergeschaltet werden; die Auswahl wird in den Einstellungen gespeichert.
  ...
  - Picker / S-Picker: kopiert eine kanonische Elementkennung in die Zwischenablage. Neu: Vollformat mit stabiler Produktkennung + exakter Instanzkennung: `<PICK:<Page>|v=1;pk=<product_key>;ik=<instance_key>>`. Legacy-Referenzen ohne `v=1` bleiben lesbar.
    - Fuer Support und Tests ergaenzt die App automatisch lesbare Referenzattribute fuer Seite/Dialog, Rolle, Label und konkrete Instanz (`data-ib-page-pickkey`, `data-ib-role-pickkey`, `data-ib-label-pickkey`, `data-ib-instance-pickkey`).
    - Nach einem Pick erscheint ein reduzierter Ergebnisdialog mit dem Modustitel `Picker` oder `Super Picker`, dem gepickten Text sowie Minimieren, Maximieren und Schliessen. Die kompakte kanonische Referenz wird beim Oeffnen automatisch in die Zwischenablage kopiert; lange Hilfetexte wie `data-help` werden nicht in den Clipboard-Text uebernommen. Falls das Kopieren im Browser blockiert wird, zeigt der Dialog einen kurzen Fehlerhinweis. Wird der S-Picker aus einem offenen Dialog gestartet, bleiben Picker-Rahmen und Ergebnisdialog im aktuellen Dialogkontext oben sichtbar.
  ...
  - Navigation zurueck/vor speichert jetzt auch die aktuelle Scroll-/Ansichtsposition. In den Einstellungen kann `Merke Position auf den Seiten` deaktiviert werden; standardmaessig wird die Position je Seite wiederhergestellt.
  ...
  - Neu: Button `Verschieben` (Icon mit 2 Quadraten + Pfeil). Dieser oeffnet den Einstellungen-Organizer.
    ...
    - Undo: stellt die letzte Layout-Aenderung wieder her (ein Schritt).
...
- Standard-Tooltips erscheinen nur noch auf wirklich interaktiven Controls wie Buttons, Checkboxen, Eingabefeldern, Auswahlfeldern und Textfeldern. Links zeigen Tooltips nur, wenn sie explizit mit `data-tooltip-allow="1"` freigegeben sind. Der Topbar-Button `Tooltips` schaltet sie seitenuebergreifend ein oder aus; die Hover-Verzoegerung ist in den Einstellungen konfigurierbar und steht standardmaessig auf 2 Sekunden. Bei einem sichtbaren Tooltip weist die kleine Fusszeile auf `Shift` hin; `Shift` friert den Tooltip ein, ein blaues Label zeigt den Freeze-Zustand, erneutes `Shift`, `Esc` oder ein Aussenklick loest ihn wieder. Picker, S-Picker und sichtbare Dialoge unterdruecken Tooltips weiterhin.
- Tooltip-Flaechen, Tooltip-Dokumentationsbutton und Query-/Info-Popup-Aktionen nutzen die Theme-Tokens der App und wechseln mit Hell/Dunkel/System-Theme mit.
- Das gewaehlte Theme wirkt seitenuebergreifend auf zentrale Flaechen, Eingaben, Buttons, Tabellen, Topbar/Bottombar, Suchergebnisse, Query-/Info-Popups und Standarddialoge. Die Standarddialoge dienen als Referenz fuer die Theme-Tokens (`--ib-surface`, `--ib-surface-2`, `--ib-border`, `--ib-text`, `--ib-primary`).
...
- Globale Dialoge wie Query-/Info-Popup, Bestaetigung, Issue Composer und Smart Bug Assistant nutzen die neue Dialogstruktur mit Kopfbereich, Beschreibung, Info/Handbuch im Header, Inhaltsbereich, Footer-Meta und Tastaturbedienung (`Esc`, `F1`, `Ctrl+I`). Das Query-/Info-Popup fasst seine Bedienaktionen in einem HA-aehnlichen Drei-Punkte-Dropdown in der Titelleiste zusammen; die Umsetzung ist nur von der Layout-Idee aus `https://github.com/djjackYT/qb-radialmenu-npstyle` inspiriert, es wurde kein Upstream-Code uebernommen. Sichtbare S-Picker-Buttons erscheinen in Dialogen nicht; der konfigurierte Shortcut bleibt dialogbezogen nutzbar.
...
### Dashboard UI-Steuerelemente

Die folgenden Eintraege sind stabile Tooltip-Dokumentationsziele. Die Referenz entspricht `data-ui` bzw. `data-ib-pickkey` im UI-Picker.
...
- Dashboard-Zustand beim Seitenwechsel: Beim Verlassen der Dashboard-Seite werden relevante Darstellungen/States im `sessionStorage` zwischengespeichert und beim erneuten Oeffnen sofort wiederhergestellt. Dazu gehoeren u.a. der Caching-Status (`load_status`), Analyse-Status/Checkliste, Ausreisser-Ergebnisliste/Selektion, Raw-Fenster sowie Graph-Detail-Settings. Das UI validiert im Hintergrund gegen den Server (Hybrid-Modus); bei veraenderten Serverdaten wird die Anzeige still aktualisiert.
  ...
  - Laeuft waehrenddessen eine Cache-Pruefung oder Analyse, bleiben die Dashboard-Sektionen `Caching` und `Analyse` jetzt trotzdem im vom Nutzer gewaehlten eingeklappten Zustand.
...
- Aktionen ueber die Buttons oberhalb der Raw-Tabelle:
  ...
  - `Reparatur-Assistent`: Wizard fuer typische Datenreparaturen im aktuell sichtbaren Raw-Fenster. Presets fuer bekannte Ausreisser-Typen (z.B. Counterreset, Grenzen, NULL/0) sowie Flatline/Duplicate-Bereinigung. Jede Ausfuehrung erfordert eine Vorschau; Bulk-Aenderungen werden als ein Undo-Block gespeichert.
...
- Der Messwertinfo-Laufzeitdialog zeigt die Server-Schritte jetzt als strukturierte Liste. Pro Schritt kann über `(i)` bzw. Inline-Details genauer nachvollzogen werden, welche Teilinformationen oder Queries die Laufzeit verursacht haben.
...
- Beim Anklicken eines Ausreißers nutzt die Raw-Ansicht vorhandene zentrierte Raw-Fenster jetzt bevorzugt aus vorhandenem Frontend-/Outlier-Cache. Dadurch werden wiederholte Datenbankabfragen für denselben bereits geladenen Outlier-Kontext deutlich reduziert.
- Dashboard-interne Nebenläufe für `Messwertinfos`, `Strategiewahl`, `Namenshistorie` und `Fields` werden bei identischer Auswahl jetzt zusammengeführt. Erwartete Abbrüche bei schnell wechselnden Auswahlen werden dabei nicht mehr als harte Fetch-Fehler protokolliert.
...
- Default: Die Ausreißer-Tabelle startet mit 15 sichtbaren Zeilen. Spaltenbreiten und Tabellenhoehe werden wie ueblich automatisch gespeichert (und koennen in UI-Profilen getrennt sein).
...
- Iconbilder: Die Tabellenkopfzeile bleibt beim vertikalen Scrollen fixiert. Zusaetzlich bleiben in der Tabelle die ersten vier Spalten (`Area`, `data-ui Key`, `Label`, `SVG (Override)`) beim horizontalen Scrollen fixiert.
- Iconbilder: Zeilenklick markiert nur noch die Zeile und setzt keine Sortierung mehr zurueck. Das Inline-Editieren in `SVG (Override)` startet mit dem aktuell sichtbaren SVG-Code; Doppelklick und `Edit` oeffnen den Editor wieder zuverlaessig und `Enter` speichert direkt.
...
- Raw Daten: Die Aktionsleiste ist in Standard- und Customer-Actions getrennt. Allgemeine Tabellenaktionen bleiben oben; spezifische Raw-Aktionen wie Kopieren, Editieren, Loeschen und Ignore/Undo liegen in einer eigenen Leiste darunter.
- Statistik: Die Messwertauswahl verwendet jetzt dasselbe Grundtemplate wie das Dashboard, damit Quelle/Zeitbereich und die zugehoerigen Hilfetexte konsistent aufgebaut sind.
...
- Support Bundle: Im Dialog koennen Snapshots erzeugt und aus einer Liste ausgewaehlt werden. Der neueste Snapshot ist standardmaessig vorausgewaehlt und wird bei Aktivierung als `snapshot.json` im Bundle abgelegt.
- Tabellen/Jobs & Cache: Die Tabellen-Info erklaert jetzt die Toolbar-/Checkbox-/Select-/Windowwidth-Funktionen vollstaendiger. Auf der Jobs-&-Cache-Seite bleiben Titel- und Action-Header oberhalb des Tabellen-Scrollbereichs fixiert.
